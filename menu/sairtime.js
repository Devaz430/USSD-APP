...
exports.sairtime =async function(mobile,msg){
...
if (msg=== '*')  {
				switch (menu.menu) {
					...
					case 'mobile':
						menu.menu = 'option';
						menu.message = lang[local].sairtimeOption
	...
	switch (menu.menu) {
		case 'option':
				if (Number(msg) === 1 ) {
					...
					menu.state ='sairtime';
					menu.message = `${lang[local].retry} ${lang[local].sairtimeOption}`
			    }	
		   break;
		case 'list':
		...
		case 'mobile':
			if(menu.permissions.length === 1){
				if( msg.length ===9 && (msg.substring(0,1) ==='9') || (msg.length ===10 && (msg.substring(0,2) ==='09')) || msg.length ===12 && (msg.substring(0,4) ==='2519') ) {
					menu.toAccount = msg.slice(-9)
					menu.fromAccount= menu.permissions[0]
					let balance =await accountlogic.accountDetails(mobile,menu.fromAccount)
					if(balance.data){
						menu.balance =balance.data.AvailableBalance
					...
					menu.message = `${lang[local].retry} ${lang[local].sairtime} ${lang[local].stopupMobile} ${lang[local].back}`
				}
				
			}else if( menu.permissions.length > 1){

				if(msg.length ===10 && (msg.substring(0,2) ==='07')) {
					menu.toAccount = msg.slice(-9)
					var message = lang[local].continue 
					menu.menu = 'list'
					for (let index = 0; index < menu.permissions.length; index++) {
		...
		case 'amount':
			if(/^\d+$/.test(msg) && Number(msg) < Number(menu.balance) && msg != '0' && (Number(msg) >= 1 && Number(msg) < 5000)){
				...
				menu.message =`${lang[local].sairtime} ${lang[local].fromAccount} ${menu.fromAccount}\n ${lang[local].mobileNumber} ${menu.toAccount}\n ${lang[local].selectedAmount}  ${menu.amount} ${lang[local].etb}\n ${lang[local].confirm}`;
					...
					menu.message = `${lang[local].Insufficient} ${lang[local].sairtime} ${lang[local].fromAccount} ${menu.fromAccount}\n ${lang[local].mobileNumber}  ${menu.toAccount} \n ${lang[local].topupamount} ${lang[local].back}`;
		    }else {
				menu.message =`${lang[local].retry} ${lang[local].sairtime} ${lang[local].fromAccount} ${menu.fromAccount}\n ${lang[local].mobileNumber} ${menu.toAccount} \n ${lang[local].topupamount} ${lang[local].back}`;
			}
			break;	
	case 'confirm':
		if(Number(msg) === 1 ){
