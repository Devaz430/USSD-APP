...
exports.transfer= async function(mobile,msg){
	...
	if(menu.permissions.length === 0){
		...
		if(msg === '1'){
            ...
            menu.message =lang[local].ownTeleBirr +`${lang[local].request}  ${lang[local].from} ${menu.fromAccount}   \n  ${lang[local].to}  ${mobile}  ${lang[local].phoneNumber} `+ lang[local].amount + lang[local].back  ;
		}else if(msg=== '3'){
			menu.fromAccount= menu.permissions[0]
